...
- 2015.01 ~ 2017.01 (関西)常駐組み込み系エンジニア
- 2017.02 ~ 2020.03 (札幌)受注ゲームバックエンドエンジニア
- 2020.04 ~ 現在 (渋谷)音声配信サービスバックエンドエンジニア
...
- 関わったプロジェクト
  - プリンタドライバ
  - プリンタ送受信ツール
  - ウェハ洗浄機のシーケンサー制御用のコンソールアプリケーション
  - ウェハ洗浄機のノズル組み合わせ表管理 Form アプリケーション
- 言語
  - C, C++ 
  - C#.Net
  - VBA
...
- バージョン管理
  ...
  - Mercurial
- OS
  - Windows 2000, XP, vista, 8, 2000NT
- IDE
  - Visual Studio 2008, 2011 ... etc

#### (札幌)受注ゲームバックエンドエンジニア

- 関わったプロジェクト
  - 社内向け週報ツール
  - ブラウザゲームのAPI/管理ツールサーバーの保守・運用
  - スマホゲームのAPI/管理/分析ツールサーバーの新規構築・保守・運用・分析
  - 負荷試験（JUnit, locust）
- 言語
  - PHP 5.6, 7.1, 7.2 ... 
  - shell
- DB
  - MySQL 5.6, 5.7
- 開発環境
  - vagrant → (途中から) docker-compose
- CI
  - Jenkins (phpunit, phpcs ... etc)
- CD
...
- インフラ
  - AWS
    - EC2 (踏み台サーバー, APIサーバー, バッチサーバー ... etc 本番のみ Auto Scaling Group)
    - Aurora (垂直, 水平分割あり)
    - Elastic Cache
    - CloudFront + S3 (リソース管理)
    - Lambda (CloudRont のヘッダー認証など）
    - sns (通知周りなど)
    - Cloud Watch (ロギング/ダッシュボード, メトリクス設定なし)
    - CodeDeploy
    - CloudFormation (IAM 管理など一部）
    - IAM
- 監視
  - アプリケーションエラー Sentry
- IDE
...
- 関わったプロジェクト
  - EKS メジャーアップデート対応
    ...
    - CD の GitOps 化
  - 課金サービス機能対応（golang)
    - API基盤作成(ログ監視, エラーハンドリング, レイヤードアーキテクチャ, ユニットテスト, E2E, dockernize ...etc)
    - CI/CD パイプライン構築
    - 機能実装
  - メインシステムの運用・保守
- 言語
...
- DB
  - MySQL 5.7
- CI
  - CircleCI (test, lint, build ... etc)
  - (一部) GitHub Actions
- CD
  - skaffold → fluxcd による pull 型 GitOps 形式
- バージョン管理
  - GitHub
- インフラ
  - AWS
...
<!--
**somen440/somen440** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.
